...
export function BottomNavigation({ page, setPage }: { page: PageId; setPage: (page: PageId) => void }) {
  return (
    <>
      <nav className="no-print fixed inset-x-0 bottom-0 z-[1000] border-t border-petrol-100 bg-white/95 px-2 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 backdrop-blur lg:hidden" aria-label="Navegación principal">
        <div className="mx-auto grid max-w-xl grid-cols-6 gap-1">
          {items.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setPage(id)} className={`animate-soft flex min-h-14 flex-col items-center justify-center rounded-xl text-xs font-semibold transition active:scale-[0.98] ${page === id ? 'bg-petrol-50 text-petrol-700 shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`} aria-current={page === id ? 'page' : undefined}>
      ...
      <aside className="no-print order-first hidden w-64 shrink-0 lg:block">
        <div className="sticky top-5 rounded-[20px] border border-app-border bg-white/95 p-3 shadow-card backdrop-blur">
