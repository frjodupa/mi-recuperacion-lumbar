      ...
      <aside className="no-print order-first hidden w-72 shrink-0 lg:block 2xl:w-80">
        <div className="sticky top-5 rounded-[34px] border border-white/70 bg-white/72 p-4 shadow-card backdrop-blur-2xl">
          <div className="mb-5 flex items-center gap-3 rounded-[26px] border border-petrol-100/70 bg-gradient-to-br from-petrol-50/90 to-white/70 p-3 shadow-soft">
            <div className="grid size-13 place-items-center rounded-[20px] bg-petrol-500 text-white shadow-[0_14px_34px_rgba(15,92,99,0.24)]">
              <SpineMark className="size-7" />
            </div>
            <div>
              <p className="text-base font-bold leading-tight tracking-[-0.02em] text-[var(--color-title)]">Mi recuperación</p>
              <p className="text-sm font-semibold text-[var(--color-text-secondary)]">Lumbar</p>
            </div>
          </div>
          ...
          {items.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setPage(id)} className={`app-nav-button animate-soft relative flex w-full items-center gap-3 rounded-[20px] px-3 py-3 text-left font-semibold transition ${page === id ? 'bg-petrol-500 text-white shadow-[0_14px_32px_rgba(15,92,99,0.22)]' : 'text-petrol-700 hover:bg-petrol-50/80 hover:pl-4'}`} aria-current={page === id ? 'page' : undefined}>
              <span className={`grid size-9 place-items-center rounded-2xl ${page === id ? 'bg-white/16 text-white' : 'bg-white/70 text-petrol-700 shadow-sm'}`}><Icon className="size-4.5" aria-hidden /></span>
              {label}
            </button>
          ))}
          </div>
          <div className="mt-5 border-t border-petrol-100/70 pt-4">
            <p className="mb-2 px-1 text-[11px] font-bold uppercase tracking-[0.22em] text-[var(--color-text-secondary)]">Modo visual</p>
