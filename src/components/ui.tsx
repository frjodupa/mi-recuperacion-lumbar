...
export function Card({ children, className = '', ...props }: HTMLAttributes<HTMLElement> & { children: ReactNode }) {
  return <section className={`print-card animate-soft glass-card premium-card-enter rounded-[30px] border p-5 sm:p-6 lg:p-7 hover:-translate-y-0.5 ${className}`} {...props}>{children}</section>;
}
...
export function Button({ children, variant = 'primary', className = '', ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'secondary' | 'ghost' | 'danger' }) {
  ...
  return <button className={`ui-button ui-button-${variant} animate-soft inline-flex min-h-[48px] items-center justify-center gap-2 rounded-[20px] px-5 py-3 text-[15px] font-semibold tracking-[-0.01em] transition disabled:cursor-not-allowed disabled:opacity-50 ${variants[variant]} ${className}`} {...props}>{children}</button>;
}
...
export function SafetyNotice() {
  return (
    <div className="safety-notice rounded-[24px] border border-amber-200/80 bg-amber-50/80 p-4 shadow-sm backdrop-blur">
      <div className="flex gap-3">
        <AlertTriangle className="warning-icon mt-0.5 size-5 shrink-0" aria-hidden />
        <p className="m-0 text-sm font-semibold leading-relaxed text-[var(--color-title)]">{safetyNotice}</p>
      </div>
    </div>
  );
}
...
export function ConfirmDialog({ title, body, onConfirm, onCancel }: { title: string; body: string; onConfirm: () => void; onCancel: () => void }) {
  return (
    <Modal title={title} onClose={onCancel}>
      <p className="mb-5 leading-relaxed text-[var(--color-text-secondary)]">{body}</p>
...
export function EmptyState({ title, body }: { title: string; body: string }) {
  return <div className="rounded-[26px] border border-dashed border-petrol-100 bg-white/48 p-8 text-center"><p className="font-bold text-[var(--color-title)]">{title}</p><p className="mt-1 text-sm text-[var(--color-text-secondary)]">{body}</p></div>;
}
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        ...
        <span className="rounded-full border border-petrol-100 bg-white/70 px-3 py-1 font-bold text-[var(--color-title)] shadow-sm">{value}/10</span>
      </div>
      ...
      <div className="mt-1 flex justify-between text-xs text-[var(--color-text-disabled)]"><span>0</span><span>10</span></div>
    </div>
  );
}
...
export function ProgressRing({ percent }: { percent: number }) {
  ...
  return (
    <div className="premium-ring grid size-32 shrink-0 place-items-center rounded-full shadow-soft" style={{ background: `conic-gradient(#2f8f69 ${clamped}%, var(--ring-track) 0)` }} aria-label={`${clamped}% completado`}>
      <div className="grid size-24 place-items-center rounded-full border border-app-border bg-app-surface/95 text-2xl font-bold tracking-[-0.03em] text-[var(--color-title)] shadow-inner">{clamped}%</div>
    </div>
  );
}
...
export function ExercisePhoto({ label, src }: { label: string; src?: string }) {
  if (src) {
    return (
      <figure className="overflow-hidden rounded-xl border border-petrol-100 bg-petrol-50">
        ...
        <figcaption className="px-3 py-2 text-center text-xs font-semibold text-[var(--color-title)]">{label}</figcaption>
      </figure>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-petrol-100 bg-petrol-50 p-3">
      ...
      <p className="mt-2 text-center text-xs font-semibold text-[var(--color-title)]">{label}</p>
    </div>
  );
}
